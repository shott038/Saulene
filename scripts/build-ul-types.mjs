...
import { BODY, EYES, INK, INK_COLOR, WISPS } from "./ul-geometry.mjs";
...
function seedOf(v, stub) {
  let h = 2166136261 >>> 0;
  const push = (n) => {
    const x = Math.floor(clamp01(n) * 65535);
    h = Math.imul(h ^ (x & 255), 16777619) >>> 0;
    h = Math.imul(h ^ (x >>> 8), 16777619) >>> 0;
  };
...
function hue(t) {
  const x = clamp01(t) * (HUES.length - 1);
  ...
  return (lerp(a, b, f) + 360) % 360;
}

function mbti(v) {
  return (
    (v.enthusiasm + v.assertiveness >= 1 ? "E" : "I") +
...
const TOP = new Set([2, 3, 4]);
const BOT = new Set([6, 7, 8]);
...
function renderUl(v, stub, size = 130, bg = "#fbf8f2") {
  ...
  const circ = (arr) =>
    arr
      .map(
        ([, , r], i) =>
          `<circle cx="${pos[i].x.toFixed(1)}" cy="${pos[i].y.toFixed(1)}" r="${(r * sr * fac(i)).toFixed(1)}"/>`,
      )
      .join("");
  ...
  const eyes = EYES.map(
    ([x]) =>
      `<circle cx="${(CX + (x - CX) * wx * gapF).toFixed(1)}" cy="${eyeY.toFixed(1)}" r="${eyeR.toFixed(1)}"/>`,
  ).join("");
  ...
  const wisps = WISPS.filter((_, i) => keep.includes(i))
    .map(([x1, x2, y]) => {
      const c = (x1 + x2) / 2;
      const hl = ((x2 - x1) / 2) * le;
...
for (const t of [
  ...
  const v = {
    openness: N ? 0.82 : 0.32,
    intellect: N ? 0.78 : 0.34,
    industriousness: J ? 0.72 : 0.34,
    orderliness: J ? 0.72 : 0.34,
    enthusiasm: E ? 0.74 : 0.3,
    assertiveness: E ? 0.7 : 0.32,
    compassion: F ? 0.8 : 0.4,
    politeness: F ? 0.5 : 0.72,
    withdrawal: 0.4 + r() * 0.2,
    volatility: 0.35 + r() * 0.3,
  };
...
const typeCards = TYPES.map(
  (t) =>
    `<figure class="card">${renderUl(t.v, t.stub, 150)}<figcaption><strong>${t.label}</strong></figcaption></figure>`,
).join("");
const popCards = POP.map(
  (p) =>
    `<figure class="card sm">${renderUl(p.v, p.stub, 116)}<figcaption>${mbti(p.v)} <span class="sex">${p.sex}</span></figcaption></figure>`,
).join("");
