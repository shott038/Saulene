...
import { WISPS, INK, BODY, INK_COLOR, PAPER } from "./ul-geometry.mjs";

// crop window around the cloud incl. the outer wind-wisps (svg units)
const X0 = 44, X1 = 256, Y0 = 42, Y1 = 160;
...
const inCircles = (circles, x, y) => circles.some(([cx, cy, r]) => (x - cx) ** 2 + (y - cy) ** 2 <= r * r);
const onWisp = (x, y) => WISPS.some(([x1, x2, wy]) => y >= wy - WISP_HALF && y <= wy + WISP_HALF && x >= x1 - WISP_HALF && x <= x2 + WISP_HALF);
...
const ring = (circles, inflate) => circles.map(([cx, cy, r], i) => [cx, cy, i === 0 ? r : r + inflate]);
    ...
    for (let c = 0; c < cols; c++) {
      ...
      for (let i = 0; i < SS; i++) {
        ...
        for (let j = 0; j < SS; j++) {
          ...
          if (col) { n++; sr += col.r; sg += col.g; sb += col.b; }
        }
      }
      // coverage threshold; internal body/ink edges blend via the averaged color
      line.push(n / (SS * SS) >= coverage ? { r: Math.round(sr / n), g: Math.round(sg / n), b: Math.round(sb / n) } : null);
    }
    px.push(line);
  }
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  const cell = (n, m) => { const g = renderDefault(n, m); return `<div class="cell"><div class="cap">${n}×${g.h} px → ${n}×${Math.ceil(g.h / 2)} chars</div>${toHtml(g)}</div>`; };
